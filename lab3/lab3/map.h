...
public:
	...
	void insert(char ket, T data);
	void find(int*check, T*finded, char key);
	void finddata(T data, char*key, int*check);
...
template<class T>
char map<T>::BfsIterator::current_key() {
...
template<class T>
inline void map<T>::insert(char key, T data)
{
	node*cur;
	if (root == NULL)
	{
...
template<class T>
inline void map<T>::find(int * check, T * finded, char key)
{
	node*cur = root;
...
template<class T>
inline void map<T>::finddata(T data, char*key, int*check)
{
	*check = -1;
...
template<class T>
inline bool map<T>::contains(char key)
{
	int check;
	T finded;
	try
...
template<class T>
inline void map<T>::remove(char key)
{
	node*cur;
	node*child;
	node*copy_cur;
